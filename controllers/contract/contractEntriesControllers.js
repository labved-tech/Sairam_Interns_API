...
exports.getAllTableContractEntries = catchAsync(async (req, res, next) => {
  let query;
  // BUILD QUERY
  // 1A) Filtering
  ...
  query = ContractEntries.find(searchObj);
  ...
  if (req.query.pagination) {
    numRecords = await ContractEntries.countDocuments(); // has to be replaced with query.countDocuments();
  ...
  // EXECUTE QUERY
  const contractEntries = await query;
  //contractEntries = await ContractEntries.find();
