...
exports.getAllTableEcommerceOrder = catchAsync(async (req, res, next) => {
  let query;
  // BUILD QUERY
  // 1A) Filtering
  ...
  query = EcommerceOrder.find(searchObj);
  ...
  if (req.query.pagination) {
    numRecords = await EcommerceOrder.countDocuments(); // has to be replaced with query.countDocuments();
  ...
  // EXECUTE QUERY
  const ecommerceOrder = await query;
  //ecommerceOrder = await EcommerceOrder.find();
  //console.log(ecommerceOrder);

  // SEND RESPONSE
  res.status(200).json({
    status: 'success',
    message: 'Got All Ecommerce Order ',
    ecommerceOrder,
    meta: {
