...
exports.getAllTableEventEntries = catchAsync(async (req, res, next) => {
  let query;
  // BUILD QUERY
  // 1A) Filtering
  ...
  query = EventEntries.find(searchObj);
  ...
  if (req.query.pagination) {
    numRecords = await EventEntries.countDocuments(); // has to be replaced with query.countDocuments();
  ...
  // EXECUTE QUERY
  const eventEntries = await query;
  //eventEntries = await EventEntries.find();
