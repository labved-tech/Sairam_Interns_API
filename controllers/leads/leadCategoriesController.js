...
exports.getAllTableLeadCategories = catchAsync(async (req, res, next) => {
  let query;
  // BUILD QUERY
  // 1A) Filtering
  ...
  query = LeadCategories.find(searchObj);
  ...
  if (req.query.pagination) {
    numRecords = await LeadCategories.countDocuments(); // has to be replaced with query.countDocuments();
  ...
  // EXECUTE QUERY
  const leadCategories = await query;
  //leadCategories = await LeadCategories.find();
