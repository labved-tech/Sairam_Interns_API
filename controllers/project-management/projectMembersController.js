...
exports.getAllTableProjectMembers = catchAsync(async (req, res, next) => {
  let query;
  // BUILD QUERY
  // 1A) Filtering
  ...
  query = ProjectMembers.find(searchObj);
  ...
  if (req.query.pagination) {
    numRecords = await ProjectMembers.countDocuments(); // has to be replaced with query.countDocuments();
  ...
  //projectMembers = await ProjectMembers.find();
  //console.log(projectMembers);

  // SEND RESPONSE
  res.status(200).json({
    status: 'success',
    message: 'Got All Project Members',
    projectMembers,
    meta: {
...
exports.getAllProjectMembers = catchAsync(async (req, res, next) => {
  console.log('Getting All Project Members');
...
exports.getProjectMembers = catchAsync(async (req, res, next) => {
  const { id } = req.params;
  console.log(`Getting Project Members for Id ${id}`);
...
exports.updateProjectMembers = catchAsync(async (req, res, next) => {
  ...
  res.status(201).json({
    status: 'success',
    message: `Updated Project Members Id=${id}`,
    data: { projectMembers },
  });
...
exports.deleteProjectMembers = catchAsync(async (req, res, next) => {
  const { id } = req.params;
  console.log(`Deleting Project Members Id ${id}`);

  const projectMembers = await ProjectMembers.findByIdAndDelete(id).then();

  res.status(200).json({
    status: 'success',
    message: `Deleted Project Members Id=${id}`,
    data: { projectMembers },
  });
