...
/* MIDDLEWARES */
const ProjectActivity= require('./../../models/project_man_n_tasks/projectActivityModel');
...
exports.updateProjectActivity= async (req, res, next) => {
  ...
  try {
    const projectActivity= await ProjectActivity.findByIdAndUpdate(id, req.body, {
      new: true,
    }).then();
