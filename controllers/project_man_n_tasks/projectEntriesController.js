...
/* MIDDLEWARES */
const ProjectEntries = require('./../../models/project_man_n_tasks/projectEntriesModel');
...
exports.getAllProjectEntries = async (req, res, next) => {
  ...
  try {
    const projectEntriess = await ProjectEntries.find().then();

    res.status(200).json({
      status: 'sucess',
      message: 'Got All projectEntries',
      results: projectEntriess.length,
      data: {
        projectEntriess,
      },
    });
...
exports.updateProjectEntries = async (req, res, next) => {
  ...
  try {
    const projectEntries = await ProjectEntries.findByIdAndUpdate(id, req.body, {
      new: true,
    }).then();
