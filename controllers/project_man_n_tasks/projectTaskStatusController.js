...
/* MIDDLEWARES */
const ProjectTaskStatus = require('./../../models/project_man_n_tasks/projectTaskStatusModel');
...
exports.updateProjectTaskStatus = async (req, res, next) => {
  ...
  try {
    const projectTaskStatus = await ProjectTaskStatus.findByIdAndUpdate(id, req.body, {
      new: true,
    }).then();
...
exports.deleteProjectTaskStatus = async (req, res, next) => {
  ...
  try {
    const projectTaskStatus = await ProjectTaskStatus.findByIdAndDelete(id).then();
