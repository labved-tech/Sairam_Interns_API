...
exports.getAllTableDeliveryNote = catchAsync(async (req, res, next) => {
  let query;
  // BUILD QUERY
  // 1A) Filtering
  ...
  query = DeliveryNote.find(searchObj);
  ...
  if (req.query.pagination) {
    numRecords = await DeliveryNote.countDocuments(); // has to be replaced with query.countDocuments();
  ...
  // EXECUTE QUERY
  const deliveryNote = await query;
  //deliveryNote = await DeliveryNote.find();
