...
exports.getAllTableTicketCategories = catchAsync(async (req, res, next) => {
  let query;
  // BUILD QUERY
  // 1A) Filtering
  ...
  query = TicketCategories.find(searchObj);
  ...
  if (req.query.pagination) {
    numRecords = await TicketCategories.countDocuments(); // has to be replaced with query.countDocuments();
  ...
  //ticketCategories = await TicketCategories.find();
  console.log(ticketCategories);

  // SEND RESPONSE
  res.status(200).json({
    status: 'success',
    message: 'Got All Ticket Categories Table',
    ticketCategories,
    meta: {
