...
exports.getAllTableTicketProducts = catchAsync(async (req, res, next) => {
  let query;
  // BUILD QUERY
  // 1A) Filtering
  ...
  query = Ticketproducts.find(searchObj);
  ...
  if (req.query.pagination) {
    numRecords = await Ticketproducts.countDocuments(); // has to be replaced with query.countDocuments();
  ...
  // EXECUTE QUERY
  const ticketproducts = await query;
  //ticketproducts = await Ticketproducts.find();
