...
exports.getAllTableTicketResponse = catchAsync(async (req, res, next) => {
  let query;
  // BUILD QUERY
  // 1A) Filtering
  ...
  query = TicketResponse.find(searchObj);
  ...
  if (req.query.pagination) {
    numRecords = await TicketResponse.countDocuments(); // has to be replaced with query.countDocuments();
  ...
  // EXECUTE QUERY
  const ticketResponse = await query;
  //ticketResponse = await TicketResponse.find();
  console.log(ticketResponse);

  // SEND RESPONSE
  res.status(200).json({
    status: 'success',
    message: 'Got All Ticket Response Table',
    ticketResponse,
    meta: {
